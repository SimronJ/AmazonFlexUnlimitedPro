...
ANDROID_FLEX_VERSION = "3.99.1.150.0"
USER_AGENT = (
    "iOS/17.0.3 (iPhone Darwin) Model/iPhone Platform/iPhone15,3 RabbitiOS/2.123.1"
)
...
class FlexUnlimited:
    ...
    def __init__(self) -> None:
        try:
            with open("config_secret.json") as configFile:
                ...
                public_key_str = config["publicAttestationKey"]
        except KeyError as nullKey:
            Log.error(
                f"{nullKey} was not set. Please set up FlexUnlimited as described in the README."
            )
            sys.exit()
        ...
        if not private_key_str or not public_key_str:
            ...
            private_key_str, public_key_str = self.serialize_and_encode_keys(
        ...
        try:
            ...
            Log.error(
                f"Error loading attestation key. Traceback: {traceback.format_exc()}"
            )
            sys.exit()
    ...
    def __registerAccount(self):
        ...
        amazon_reg_data = {
            ...
            "user_context_map": {"frc": self.__generate_frc(self.android_device_id)},
        }
        res = self.session.post(
            FlexUnlimited.routes.get("GetAuthToken"),
            json=amazon_reg_data,
            headers=self.allHeaders.get("AmazonApiRequest"),
            verify=True,
        )
    ...
    def serialize_and_encode_keys(
        ...
        serialized_public_key = FlexUnlimited.serialize_public_key(public_key)
        b64_encoded_private_key = FlexUnlimited.encode_key(serialized_private_key)
        b64_encoded_public_key = FlexUnlimited.encode_key(serialized_public_key)
    ...
    def load_attestation_private_key(private_key: str) -> ec.EllipticCurvePrivateKey:
        ...
        loaded_private_key = serialization.load_der_private_key(
            decoded_private_key, password=None, backend=default_backend()
        )
    ...
    def register_attestation(self):
        ...
        if response.status_code != 200:
            print(
                f"Error sending request to register-attestation. Status code: {response.status_code}"
            )
        ...
        self.__update_config_file(
            {"keyId": self.key_id, "keyIdExpiration": self.key_id_expiration}
        )

    @staticmethod
    def __update_config_file(key_pairs: dict):
        try:
            with open("config_secret.json", "r+") as configFile:
                config = json.load(configFile)
    ...
    def run(self):
        ...
        while self.__retryCount < self.retryLimit:
            sleeptime = self.refreshInterval
            if not self.__retryCount % 50:
                print(self.__retryCount, "requests attempted\n")
            if (
                self.__accept_headers_last_updated
                < time.time() - REFRESH_SIGNATURE_INTERVAL * 60
            ):
                self.sign_accept_headers()
            ...
            if offersResponse.status_code == 200:
                ...
                for offer in currentOffers:
                    ...
                    Log.notice(
                        "Found the following offer (now sleeping "
                        + str(sleeptime)
                        + "s):"
                    )
                    Log.notice(offerResponseObject.toString())
                self.__retryCount += 1

                if len(currentOffers) == 0:
                    Log.notice(
                        str(self.__retryCount)
                        + ": Found 0 new offer(s), sleeping "
                        + str(sleeptime)
                        + "s."
                    )
