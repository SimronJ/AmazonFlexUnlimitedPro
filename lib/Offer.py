...
class Offer:
    def __init__(self, offerResponseObject: object) -> None:
        ...
        self.expirationDate = datetime.fromtimestamp(
            offerResponseObject.get("expirationDate")
        )
        ...
        self.ratePerHour = round(
            self.blockRate / ((self.endTime - self.startTime).seconds / 3600), 2
        )
    ...
    def toString(self) -> str:
        ...
        body += (
            "Date: " + str(self.startTime.month) + "/" + str(self.startTime.day) + "\n"
        )
        ...
        body += (
            "Block Duration: "
            + str(blockDuration)
            + f'{"hour" if blockDuration == 1 else "hours"}\n'
        )
        ...
        if not self.startTime.minute:
            ...
            body += (
                "Start time: "
                + str(self.startTime.hour)
                + "0:"
                + str(self.startTime.minute)
                + "\n"
            )
        else:
            body += (
        ...
        if not self.endTime.minute:
            ...
            body += (
                "End time: "
                + str(self.endTime.hour)
                + "0:"
                + str(self.endTime.minute)
                + "\n"
            )
        else:
            body += (
